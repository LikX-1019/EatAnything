...
const form = reactive({ schoolCode: '', name: '', city: '', district: '', address: '', status: 'active' })
...
function openCreate() { editingId.value=''; Object.assign(form,{schoolCode:'',name:'',city:'',district:'',address:'',status:'active'}); dialog.value=true }
function openEdit(item: SchoolOption & Record<string, any>) { editingId.value=item.id; Object.assign(form,{schoolCode:item.schoolCode,name:item.name,city:item.city||'',district:item.district||'',address:item.address||'',status:item.status}); dialog.value=true }
async function save() { try { if(editingId.value) await api.patch(`/admin/schools/${editingId.value}`,form); else await api.post('/admin/schools',form); ElMessage.success('学校资料已保存'); dialog.value=false; await load() } catch(e){ ElMessage.error(e instanceof Error?e.message:'保存失败') } }
...
<template>
  ...
  <el-dialog v-model="dialog" :title="editingId?'编辑学校贴纸':'新增学校贴纸'" width="600px"><el-form label-position="top"><div class="form-grid"><el-form-item label="学校编码"><el-input v-model="form.schoolCode" :disabled="Boolean(editingId)" /></el-form-item><el-form-item label="学校名称"><el-input v-model="form.name" /></el-form-item><el-form-item label="城市"><el-input v-model="form.city" /></el-form-item><el-form-item label="区县"><el-input v-model="form.district" /></el-form-item><el-form-item label="地址" class="wide"><el-input v-model="form.address" /></el-form-item><el-form-item label="状态"><el-select v-model="form.status"><el-option label="启用" value="active"/><el-option label="停用" value="hidden"/></el-select></el-form-item></div></el-form><template #footer><el-button @click="dialog=false">取消</el-button><el-button type="primary" @click="save">保存</el-button></template></el-dialog>
