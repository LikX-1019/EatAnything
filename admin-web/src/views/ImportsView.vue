...
const file=ref<File|null>(null);const loading=ref(false);const result=ref<Result|null>(null);const error=ref('');const history=ref<Array<Record<string,any>>>([])
...
async function submit(){if(!file.value)return ElMessage.warning('请先选择 CSV 或 XLSX 文件');const data=new FormData();data.append('file',file.value);loading.value=true;error.value='';try{result.value=await api.post<Result>('/admin/stores/import',data);ElMessage.success('批量导入完成');await loadHistory()}catch(e){error.value=e instanceof Error?e.message:'导入失败'}finally{loading.value=false}}
function template(){const csv='storeCode,schoolCode,areaCode,name,category,address,imageUrl,status\nexample-001,school-code,area-code,示例店铺,简餐,示例地址,,hidden\n';const url=URL.createObjectURL(new Blob(['\ufeff'+csv],{type:'text/csv;charset=utf-8'}));const a=document.createElement('a');a.href=url;a.download='店铺导入模板.csv';a.click();URL.revokeObjectURL(url)}
onMounted(loadHistory)
</script>
<template>
  <section class="paper-card upload-scrap"><div><div class="upload-icon">📎</div><h3>把店铺表格贴进管理手账</h3><p>支持 UTF-8 CSV 或 XLSX，最多 1000 行；任何一行有误都会整批回滚。</p><el-upload :auto-upload="false" :limit="1" accept=".csv,.xlsx" :on-change="choose"><el-button type="primary">选择文件</el-button></el-upload><p v-if="file"><strong>{{ file.name }}</strong> · {{ (file.size/1024).toFixed(1) }} KB</p><div><el-button text @click="template">下载 CSV 模板</el-button><el-button type="success" :loading="loading" :disabled="!file" @click="submit">校验并导入</el-button></div><div v-if="error" class="error-note">✎ {{ error }}</div></div></section>
