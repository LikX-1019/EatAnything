...
<template>
  <view class="page-shell history-page" :class="appStore.fontClass"><PageHeader title="历史记录" back /><view class="page-pad"><view v-if="loading" class="page-state">正在加载历史记录…</view><view v-else-if="errorMessage" class="page-state"><text>{{ errorMessage }}</text><button class="retry-button" @tap="loadHistory()">重新加载</button></view><template v-else><view v-if="appStore.history.length" class="timeline"><view v-for="item in appStore.history" :key="item.id" class="history-item"><view class="dot green" /><FallbackImage class="history-image" :src="storeImageUrl(item.store)" /><view class="history-copy"><text class="history-name">{{ item.store.name }}</text><text class="history-action">已选择就餐 · {{ item.store.area || '未分区' }}</text><text class="history-date">{{ formatHistoryTime(item.occurredAt) }}</text></view></view></view><EmptyState v-else title="暂无已选择的店铺" description="点击抽签结果下方的“就吃这家！”后会显示在这里" /></template></view></view>
</template>
