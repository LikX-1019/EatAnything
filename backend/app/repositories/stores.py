...
async def count_stores(session: AsyncSession, *, active_only: bool, keyword: str | None = None, status: str | None = None) -> int:
...
async def random_store_id(session: AsyncSession, exclude_store_id: int | None = None) -> int | None:
    from sqlalchemy import func as sql_func

    query = select(Store.id).where(Store.status == "active")
    if exclude_store_id is not None:
        candidates = await session.scalar(select(func.count(Store.id)).where(Store.status == "active"))
